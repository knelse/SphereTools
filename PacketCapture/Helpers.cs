public class WorldCoords
{
    ...
    public string ToFileDumpString()
    {
        ...
        return $"{dt}\t\t\t{x:F14}\t\t\t{y:F14}\t\t\t{z:F14}\t\t\t{turn:F14}\n";
    }

    public bool Equals(WorldCoords c)
    {
        return Math.Abs(c.x - x) <= double.Epsilon
               && Math.Abs(c.y - y) <= double.Epsilon
               && Math.Abs(c.z - z) <= double.Epsilon
               && Math.Abs(c.turn - turn) <= double.Epsilon;
    }
}
